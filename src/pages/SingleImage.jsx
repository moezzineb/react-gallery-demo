      ...
      <div
        ...
        <div>
          <Box sx={{ p: 2, display: 'flex' }}>
            <Avatar variant="rounded" src={`${state?.image?.user?.profile_image?.medium}`} />
            <Stack spacing={0.8} sx={{ marginLeft: '10px' }}>
              <Typography fontWeight={700}>{`${state?.image?.user?.username}`}</Typography>
              {state?.image?.description && (
                <Typography fontWeight={500}>{`${state?.image?.description}`}</Typography>
              )}
            </Stack>
          </Box>
          <Box sx={{ display: 'flex' }}>
